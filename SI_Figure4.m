...
close all;
clc;
tableSD = readtable('State-Data-wave2.csv');
...
positon_6p3 = [889,107,680,879];%6×3

gcf1 = figure('position',positon_6p3);
gcf_t1 = tiledlayout(6,3,'TileSpacing','tight','Padding','tight');
nn = 0;
inputarea = [];
for k = 1:length(CountryD)
    ...
    clearvars -except tableSD tableConfirmed tableDeaths tableRecovered...
         CountryD k gcf1 gcf_t1 inputarea
    ...
    plot(time0(startpoint:endpoint),CDR_wave(startpoint:endpoint),'ro','markersize',3,'LineWidth',0.5);
...
lgd.Layout.Tile = 18;
title(gcf_t1,'17 areas in the second wave','FontWeight','bold','FontSize',16)
